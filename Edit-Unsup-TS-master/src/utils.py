...
def unmask(sentence, max_output_size):
    inp = mlm_tokenizer.encode(sentence, return_tensors="pt")
    mask_token_index = torch.where(inp == mlm_tokenizer.mask_token_id)[1]
    token_logits = mlm_model(inp).logits
    mask_token_logits = token_logits[0, mask_token_index, :]
    topk_tokens = torch.topk(
        mask_token_logits, max_output_size, dim=1).indices[0].tolist()
    topk_tokens = [mlm_tokenizer.decode(token) for token in topk_tokens]
    return [token for token in topk_tokens if token.isalnum()]
...
if config['lexical_simplification']:
    ...
    mlm_tokenizer = AutoTokenizer.from_pretrained(config['mlm_name_or_dir'])
    mlm_model = AutoModelForMaskedLM.from_pretrained(config['mlm_name_or_dir'])

    # mlm_tokenizer.save_pretrained('./local_model_directory/')
    # mlm_model.save_pretrained('./local_model_directory/')
...
class Lang:
    ...
    def readLangs(dataset):
        ...
        if dataset == 'Wikilarge':
            ...
            train_src = open('../data-simplification/wikilarge/wiki.full.aner.ori.train.src',
                             encoding='utf-8').read().split('\n')
            train_dst = open('../data-simplification/wikilarge/wiki.full.aner.ori.train.dst',
                             encoding='utf-8').read().split('\n')
            valid_src = open('../data-simplification/wikilarge/wiki.full.aner.ori.valid.src',
                             encoding='utf-8').read().split('\n')
            valid_dst = open('../data-simplification/wikilarge/wiki.full.aner.ori.valid.dst',
                             encoding='utf-8').read().split('\n')
            test_src = open('../data-simplification/wikilarge/wiki.full.aner.ori.test.src',
                            encoding='utf-8').read().split('\n')
            test_dst = open('../data-simplification/wikilarge/wiki.full.aner.ori.test.dst',
                            encoding='utf-8').read().split('\n')

        elif dataset == 'Newsela':
            print('loading Newsela data')
            train_src = open('../data-simplification/newsela/V0V4_V1V4_V2V4_V3V4_V0V3_V0V2_V1V3.aner.ori.train.src',
                             encoding='utf-8').read().split('\n')
            train_dst = open('../data-simplification/newsela/V0V4_V1V4_V2V4_V3V4_V0V3_V0V2_V1V3.aner.ori.train.dst',
                             encoding='utf-8').read().split('\n')
            valid_src = open('../data-simplification/newsela/V0V4_V1V4_V2V4_V3V4_V0V3_V0V2_V1V3.aner.ori.valid.src',
                             encoding='utf-8').read().split('\n')
            valid_dst = open('../data-simplification/newsela/V0V4_V1V4_V2V4_V3V4_V0V3_V0V2_V1V3.aner.ori.valid.dst',
                             encoding='utf-8').read().split('\n')
            test_src = open('../data-simplification/newsela/V0V4_V1V4_V2V4_V3V4_V0V3_V0V2_V1V3.aner.ori.test.src',
                            encoding='utf-8').read().split('\n')
            test_dst = open('../data-simplification/newsela/V0V4_V1V4_V2V4_V3V4_V0V3_V0V2_V1V3.aner.ori.test.dst',
                            encoding='utf-8').read().split('\n')

        train_simple, train_simple_unique = Lang.getSentences(
            train_dst, config['lm_backward'])
        valid_simple, valid_simple_unique = Lang.getSentences(
            valid_dst, config['lm_backward'])
        test_simple, test_simple_unique = Lang.getSentences(
            test_dst, config['lm_backward'])
        train_complex, train_complex_unique = Lang.getSentences(
            train_src, config['lm_backward'])
        valid_complex, valid_complex_unique = Lang.getSentences(
            valid_src, config['lm_backward'])
        test_complex, test_complex_unique = Lang.getSentences(
            test_src, config['lm_backward'])
...
def get_embedding_matrix(embeddings_index, vocab, embedding_dim):
    ...
    if embeddings_index is not None:
        ...
        embedding_matrix[0] = np.zeros(
            (1, embedding_dim), dtype='float32')  # PAD
        embedding_matrix[1] = np.random.uniform(
            low=-0.05, high=0.05, size=(1, embedding_dim))  # SOS
        embedding_matrix[2] = np.random.uniform(
            low=-0.05, high=0.05, size=(1, embedding_dim))  # EOS
        embedding_matrix[3] = np.random.uniform(
            low=-0.05, high=0.05, size=(1, embedding_dim))  # UNK
        l = 4
        for key in vocab:
            ...
            if embedding_vector is not None:
                ...
                embedding_matrix[l] = np.random.uniform(
                    low=-0.05, high=0.05, size=(1, embedding_dim))
            l += 1
        ...
        l = 4
        for key in vocab:
            embedding_matrix[l] = np.random.normal(
                loc=0, scale=0.1, size=(1, embedding_dim))
            l += 1
        return embedding_matrix
...
def convert_to_sent(sent):
    return ' '.join([token for token in sent if token != ''])
...
def prepareData(embedding_dim, freq, ver, dataset, operation):
    ...
    print("Trimmed to %s unique train sentence pairs" %
          len(train_simple_unique))
    print("Trimmed to %s unique valid sentence pairs" %
          len(valid_simple_unique))
    ...
    if os.path.isfile(config['dataset'] + '/outputword2count.npy'):
        ...
        outputword2count = np.load(
            config['dataset'] + '/outputword2count.npy', allow_pickle=True).item()
        ...
        output_vocab, outputword2count = getvocab(
            train_simple_unique, freq, freq, [], {})
    ...
    unigram_prob = get_unigram_probability(
        outputword2count, len(train_simple_unique))
    ...
    if not config['elmo']:
        ...
        output_embedding_weights = get_embedding_matrix(
            embeddings_index, output_vocab, embedding_dim)
    else:
        output_embedding_weights = get_embedding_matrix(
            None, output_vocab, embedding_dim)
    tag_embedding_weights = get_embedding_matrix(
        None, tag_vocab, config['tag_dim'])
    dep_embedding_weights = get_embedding_matrix(
        None, dep_vocab, config['dep_dim'])
...
def calculateProbabilitySentence(prob, sent):
    ...
    for i in range(l):
        ...
        p += prob[i][sent[0][i + 1]]
        worst_three = prob[i][sent[0][i - 1]] + \
            prob[i][sent[0][i]] + prob[i][sent[0][i + 1]]
...
def calculateLossWord(decoder, output_tensor, lang, p):
    ...
    with torch.no_grad():
        ...
        # print(loss)
        # print(math.exp(loss))
        b = torch.Tensor(1, output_tensor_len.item() -
                         1, lang.n_words).to(device)
...
def calculateLoss(decoder, elmo_tensor, output_tensor, tag_tensor, dep_tensor, lang, p):
    ...
    with torch.no_grad():
        ...
        for di in range(output_tensor_len.item() - 1):
            # -2 because of removing the SOS and EOS tokens
            decoder_output, decoder_hidden = decoder(
                decoder_input, tag_input, dep_input, decoder_hidden, False)
            indices = torch.tensor([di + 1], device=device)
            # print(output_tensor)
            target_t = torch.index_select(output_tensor, 1, indices)
            # print(target_t)
...
def get_sentence_probability(lm_forward, elmo_tensor, input_sent_tensor, tag_tensor, dep_tensor, input_lang, input_sent, unigram_prob):
    ...
    prob, _ = calculateLoss(lm_forward, elmo_tensor, input_sent_tensor,
                            tag_tensor, dep_tensor, input_lang, False)
    prob, worst_three = calculateProbabilitySentence(prob, input_sent_tensor)
    if config['SLOR']:
        slor = prob - \
            calcluate_unigram_probability(input_sent, unigram_prob, input_lang)
...
def tokenize_sent_special(input_sent, input_lang, tag_sent, tag_lang, dep_sent, dep_lang):
    ...
    input_tensor, tag_tensor, dep_tensor = removeUNKforall(
        input_tensor, tag_tensor, dep_tensor)
    # print(input_tensor)
    # input_tensor = removeUNK(input_tensor)
    # print(input_tensor)
    elmo_tensor = []
    if config['elmo']:
        ...
        elmo_tensor = pad_sequences(
            elmo_tensor[0], config['MAX_LENGTH'], True).unsqueeze(0)
    input_tensor = pad_sequences(
        input_tensor, config['num_steps'], False).unsqueeze(0)
    tag_tensor = pad_sequences(
        tag_tensor, config['num_steps'], False).unsqueeze(0)
    dep_tensor = pad_sequences(
        dep_tensor, config['num_steps'], False).unsqueeze(0)
...
def sentence_cosine_similarity(new_sentences, old_emb, old_norm):
    new_embs = nli_model.encode(new_sentences)
    magnitudes = np.sum(np.abs(new_embs)**2, axis=-1)**(1. / 2)
    magnitudes = magnitudes * old_norm
    return np.dot(new_embs, old_emb) / magnitudes
...
def delete_leaves(words, delete_words):
    main_sentence = convert_to_sent(words)
    del_phrase = convert_to_sent(delete_words) + ' '
    new_sentence = main_sentence.replace(del_phrase, '')
    # delete extra spaces
    new_sentence = re.sub(' +', ' ', new_sentence).strip()
    return correct(new_sentence)
...
def get_subphrase_mod(sent, sent_list, input_lang, idf, simplifications, entities, synonym_dict):
    ...
    try:
        ...
        print('Did not parse! CoreNLP timeout.')
    # print(tree)
    if tree:
...
def generate_phrases(sent, tree, sent_list, input_lang, idf, simplifications, entities, synonym_dict):
    ...
    for i in range(len(p)):
        if config['lexical_simplification']:
            simple = lexical_simplification(
                sent, p[i], input_lang, idf, simplifications, entities, synonym_dict)
            for st in simple:
                if st not in sent_list:
                    s.append({st: 'ls'})
        if config['delete_leaves']:
            # new
            sd = delete_leaves(tokens, p[i])
            # old
...
def reorder_leaves(words, tree_phrases):
    tree_phrases = [leaves for leaves in tree_phrases if len(leaves) > 1]
    output_sentences = set()
    for current_leaves in tree_phrases:
        target_phrase = convert_to_sent(current_leaves) + ' '
        main_sentence = convert_to_sent(words)
        clean_sentence = main_sentence.replace(target_phrase, '')[:-2]
        for some in tree_phrases:
            some_phrase = convert_to_sent(some) + ' '
            if (some_phrase not in target_phrase) and (target_phrase not in some_phrase):
                # sentence is "start ... some_phrase ... end"
                sandwich = clean_sentence.split(some_phrase, 1)
                if len(sandwich) == 2:
                    # final_sentences should be :
                    # [start, some_phrase, target_phrase, end]
                    # [target_phrase, start, some_phrase, end]
                    final_sentences = []
                    final_sentences.append(
                        [sandwich[0], some_phrase, target_phrase, sandwich[1]])
                    final_sentences.append(
                        [target_phrase, sandwich[0], some_phrase, sandwich[1]])
                    # clean-up
                    for final in final_sentences:
                        final_phrase = convert_to_sent(final)
                        final_phrase = re.sub(
                            ' +', ' ', final_phrase).strip() + ' .'
                        final_phrase = correct(final_phrase)
                        output_sentences.add(final_phrase)

    if main_sentence in output_sentences:
        output_sentences.remove(main_sentence)
...
#             except:
#                 print(
#                     'some issue as there is a mismatch of tokenization, corenlp does not always use spaces')
...
def lexical_simplification(sent, phrase, input_lang, idf, orig_sent_words, entities, synonym_dict):
    ...
    s = []
    synonyms = []
    word_to_be_replaced = get_word_to_simplify(
        phrase, idf, orig_sent_words, entities, input_lang)
    ...
    if word_to_be_replaced != '':
        if word_to_be_replaced in synonym_dict:
            ...
            masked_phrase = ''
            if sent_lowered.count(word_to_be_replaced) < 2:
                masked_phrase = sent_lowered.replace(
                    word_to_be_replaced, mlm_tokenizer.mask_token)
            else:
                joined_phrase = convert_to_sent(phrase).lower()
                masked_phrase = joined_phrase.replace(
                    word_to_be_replaced, mlm_tokenizer.mask_token, 1)
                masked_phrase = sent_lowered.replace(
                    joined_phrase, masked_phrase, 1)
                if mlm_tokenizer.mask_token not in masked_phrase:
                    masked_phrase = sent_lowered.replace(
                        word_to_be_replaced, mlm_tokenizer.mask_token, 1)

            sim = unmask('{0} [SEP] {1}'.format(sent, masked_phrase),
                         config['mlm_outputs'])
            # sim = unmask('{0} [SEP] {1}'.format(sent_lowered, masked_phrase),
            #              config['mlm_outputs'])
        ...
        checks_for_word_simplification(
            sent, word_to_be_replaced, synonyms, input_lang, pos, dep, idf, orig_sent_words, s)
        # print('s is')
        # print(s)
    return s
...
def calculate_score(lm_forward, elmo_tensor, tensor, tag_tensor, dep_tensor, input_lang, input_sent, orig_sent, embedding_weights, idf, unigram_prob, cs=1.0):

    # if cs:
    #     prob *= cos_similarity(input_sent.lower(), orig_sent.lower(), idf)

    prob = 1.0
    if cs < config['cos_similarity_threshold']:
        return 0
    else:
        prob *= cs

    token_count = len(input_sent.split(' '))
    if config['check_min_length']:
        if token_count < config['min_length_of_edited_sent']:
            return 0

    prob = get_sentence_probability(lm_forward, elmo_tensor, tensor, tag_tensor, dep_tensor,
                                    input_lang, input_sent, unigram_prob)**config['sentence_probability_power']

    prob /= token_count**config['len_power']
    prob *= (get_named_entity_score(input_sent)
             )**config['named_entity_score_power']
    if config['fre']:
